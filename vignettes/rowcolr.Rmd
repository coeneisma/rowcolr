...
Values can be extracted from an Excel file using a regex patterns. The default patterns are `.*_row$` and `.*_col$`, which means the function looks for values that end with `_row` and `_col`:
...
By default, row and column labels are cleaned by removing suffix patterns before combining them into the description. To disable this, use `clean_description = FALSE`:
